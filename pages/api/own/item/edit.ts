...
import { Category, EditItem, ItemSafe, ItemType, VisibilityLevel } from 'lib/types/item';
import { Category as PrismaCategory, VisibilityLevel as PrismaVisibilityLevel, ItemType as PrismaItemType} from '@prisma/client';
    ...
    try {
      ...
      const result = await prisma.item.update({
        ...
        data: {
          ...
          category: reqBody.category ?? undefined,
          category_id: reqBody.category_id ?? undefined,
          item_type: PrismaItemType[reqBody.item_type.toLowerCase() as keyof typeof PrismaItemType],
          date_tz_sensitive: reqBody.date_tz_sensitive ?? undefined,
          date_tz_sensitive_end: reqBody.date_tz_sensitive_end ?? undefined,
          time_sensitive_flag: reqBody.time_sensitive_flag,
          date_range_flag: reqBody.date_range_flag,
          date_tz_insensitive: reqBody.date_tz_insensitive ?? undefined,
          date_tz_insensitive_end: reqBody.date_tz_insensitive_end ?? undefined,
      ...
      const ItemSafe: ItemSafe = {
        ...
        item_type: ItemType[result.item_type as keyof typeof ItemType],
        ...
        permission_level: VisibilityLevel[result.permission_level as keyof typeof VisibilityLevel],
        created_by_id: result.created_by_id,
        last_modified_by_id: result.last_modified_by_id,
      }
      return res.json(ItemSafe)
    } catch (error) {
      console.log(error)
